...
import {
    ...
    Zap, Gauge, Settings, X, Layers, Loader2, Camera, Maximize2, Send, User, Phone, Mail, MessageSquare, Target, Activity
} from 'lucide-react';
import Footer from "@/components/layout/Footer";

export default function TakimSikmaPage() {
        ...
        <main className="bg-[#050505] min-h-screen relative text-white selection:bg-amber-500 selection:text-black font-sans overflow-x-hidden">
            ...
            <div className="fixed inset-0 w-full h-screen z-0">
                <motion.div style={{ scale }} className="relative w-full h-full">
                    <Image src={mainImage} alt="Takım Sıkma Mekanizması" fill className="object-cover opacity-40" priority />
            ...
            <div className="relative z-10 w-full">
                ...
                <section className="h-screen flex flex-col items-center justify-center text-center px-6">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
                        ...
                        <p className="text-white/80 text-xl md:text-3xl max-w-4xl mx-auto font-black leading-relaxed italic tracking-tight px-6 uppercase">
                            Hassas Sabitleme İçin Tasarlandı. Belirlenen tork değerinde otomatik duruş ve güvenli sabitleme.
                        </p>

                        <div className="flex flex-wrap justify-center gap-4 mt-12">
                            {["HASSAS TORK KONTROLÜ", "MODÜLER KULLANIM", "ANLIK ÖLÇÜM"].map((chip, i) => (
                ...
                <section className="py-32 px-6 container mx-auto">
                    <div className="mb-24 text-center">
                        <h2 className="text-6xl md:text-[10rem] font-black uppercase tracking-tighter mb-6 italic leading-none text-white">SİSTEM <span className="text-amber-500 font-light italic">AVANTAJLARI</span></h2>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {[
                            { icon: <Target size={32} />, title: "TORK KONTROLÜ", desc: "Takım başlıkları belirlenen değerlerde güvenli ve kontrollü şekilde sıkılır." },
                            { icon: <Activity size={32} />, title: "ANLIK ÖLÇÜM", desc: "Entegre ölçüm birimi ile uygulanan kuvvet anlık olarak takip edilebilir." },
                            { icon: <Layers size={32} />, title: "MODÜLER YAPI", desc: "Değiştirilebilir kovan sistemi sayesinde farklı takım tiplerine uyum sağlar." },
                            { icon: <ShieldCheck size={32} />, title: "DAYANIKLILIK", desc: "Yoğun endüstriyel kullanıma uygun, çelik ve alüminyum alaşımlı sağlam gövde." }
                        ].map((adv, i) => (
                ...
                <section className="py-32 px-6 container mx-auto max-w-4xl">
                    ...
                    <div className="grid grid-cols-1 gap-4 font-sans text-white">
                        {[
                            { l: "Kullanım Şekli", v: "Sabit Zemine Montaj" },
                            { l: "Uyumlu Takımlar", v: "Değiştirilebilir Kovan Sistemi" },
                            { l: "Gövde Materyali", v: "Çelik / Alüminyum Alaşım" },
                            { l: "Ölçüm Birimi", v: "Entegre Tork Göstergesi" }
                        ].map((item, i) => (
                ...
                <section className="py-32 px-6 container mx-auto">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {gallery.map((img, i) => (
                            ...
                            >
                                <Image src={img} alt="Takım Sıkma Galeri" fill className="object-cover grayscale hover:grayscale-0 transition-all duration-1000" />
                ...
                <section className="py-44 text-center bg-white text-black rounded-t-[6rem] md:rounded-t-[10rem] relative overflow-hidden">
                    <div className="relative z-10 px-6 font-sans">
                        <h2 className="text-7xl md:text-[12rem] font-black uppercase tracking-tighter mb-20 leading-[0.8] italic text-slate-900">GÜVENLİ <br /> <span className="text-amber-500">SABİTLEME.</span></h2>
                        <div className="flex flex-col sm:flex-row gap-6 justify-center max-w-4xl mx-auto">
                            <a href="/dokumanlar/takimsikma.pdf" target="_blank" className="flex-1 bg-slate-100 text-slate-900 px-12 py-8 rounded-full font-black text-xl hover:bg-slate-200 transition-all flex items-center justify-center gap-4 group italic border border-slate-200 uppercase tracking-widest">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[28vw] font-black text-black/[0.03] select-none pointer-events-none uppercase italic leading-none">TORQUE</div>
                </section>
            </div>
                ...
                {selectedImg && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 md:p-20">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="relative w-full h-full">
                            <Image src={selectedImg} alt="Görsel" fill className="object-contain" />
                                    ...
                                    <>
                                        ...
                                        <form onSubmit={handleFormSubmit} className="space-y-4">
                                            ...
                                            <div className="relative group text-slate-900">
                                                ...
                                                <textarea rows={2} placeholder="Proje Notlarınız..." value={form.mesaj} onChange={(e) => setForm({ ...form, mesaj: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl pl-16 pr-8 py-6 text-slate-900 outline-none focus:bg-white focus:border-amber-500 transition-all font-black resize-none" />
                                            </div>
