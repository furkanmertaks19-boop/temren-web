...
import {
    ...
    Zap, Gauge, Settings, X, Layers, Loader2, Camera, Maximize2, Send, User, Phone, Mail, MessageSquare, Sparkles, RefreshCw
} from 'lucide-react';
import Footer from "@/components/layout/Footer";

export default function KonikTemizlemePage() {
    ...
    const mainImage = "/image/konik_1.webp";
    const gallery = ["/image/konik_1.webp", "/image/konik_2.webp", "/image/konik_3.jpg"];
        ...
        <main className="bg-[#050505] min-h-screen relative text-white selection:bg-amber-500 selection:text-black font-sans overflow-x-hidden">
            ...
            <div className="fixed inset-0 w-full h-screen z-0">
                <motion.div style={{ scale }} className="relative w-full h-full">
                    <Image src={mainImage} alt="Konik Temizleme" fill className="object-cover opacity-40" priority />
            ...
            <div className="relative z-10 w-full">
                ...
                <section className="h-screen flex flex-col items-center justify-center text-center px-6">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
                        ...
                        <p className="text-white/80 text-xl md:text-3xl max-w-4xl mx-auto font-black leading-relaxed italic tracking-tight px-6 uppercase text-center">
                            Yüksek Hassasiyet İçin Derinlemesine Temizlik. Fener mili ve takım ömrünü koruyan endüstriyel çözüm.
                        </p>

                        <div className="flex flex-wrap justify-center gap-4 mt-12">
                            {["ETKİLİ TEMİZLİK", "HASSASİYET KORUMA", "OPERATÖR DOSTU"].map((chip, i) => (
                ...
                <section className="py-32 px-6 container mx-auto">
                    <div className="mb-24 text-center">
                        <h2 className="text-6xl md:text-[10rem] font-black uppercase tracking-tighter mb-6 italic leading-none text-white text-center">NEDEN <span className="text-amber-500 font-light italic">TEMİZLİK?</span></h2>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-left">
                        {[
                            { icon: <Sparkles size={32} />, title: "ETKİLİ ARINDIRMA", desc: "Reçineleşmiş kir, yağ ve sıvı kalıntılarını konik yüzeyden derinlemesine temizler." },
                            { icon: <ShieldCheck size={32} />, title: "SALGI İYİLEŞTİRME", desc: "Takım salgısını minimize ederek fener mili ve konik yüzeyi deformasyona karşı korur." },
                            { icon: <RefreshCw size={32} />, title: "EKİPMAN ÖMRÜ", desc: "Takım ve makine parçalarının kullanım ömrünü uzatarak bakım maliyetlerini düşürür." },
                            { icon: <Settings size={32} />, title: "PRATİK KULLANIM", desc: "Hızlı ve kolay uygulama adımlarıyla her seviyedeki operatör için uygundur." }
                        ].map((adv, i) => (
                ...
                <section className="py-32 px-6 container mx-auto max-w-4xl">
                    ...
                    <div className="grid grid-cols-1 gap-4 font-sans text-white">
                        {[
                            { l: "Kabin Yapısı", v: "Modüler Değiştirilebilir Sistem" },
                            { l: "Tahrik Ünitesi", v: "Sabit Motor Ünitesi" },
                            { l: "Uyumluluk", v: "BT / SK / CAT Standartları" },
                            { l: "Gövde Malzemesi", v: "Alüminyum & Polimer Kompozit" }
                        ].map((item, i) => (
                            <div key={i} className="flex justify-between items-center py-10 border-b border-white/10 group transition-all hover:bg-white/[0.02] hover:px-8 rounded-3xl text-left md:text-left">
                ...
                <section className="py-32 px-6 container mx-auto">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                        {gallery.map((img, i) => (
                            ...
                            >
                                <Image src={img} alt="Konik Galeri" fill className="object-cover grayscale hover:grayscale-0 transition-all duration-1000" />
                ...
                <section className="py-44 text-center bg-white text-black rounded-t-[6rem] md:rounded-t-[10rem] relative overflow-hidden">
                    <div className="relative z-10 px-6 font-sans">
                        <h2 className="text-7xl md:text-[12rem] font-black uppercase tracking-tighter mb-20 leading-[0.8] italic text-slate-900">SIFIR <br /> <span className="text-amber-500">HASSASİYET.</span></h2>
                        <div className="flex flex-col sm:flex-row gap-6 justify-center max-w-4xl mx-auto">
                            <a href="/dokumanlar/koniktemizleme.pdf" target="_blank" className="flex-1 bg-slate-100 text-slate-900 px-12 py-8 rounded-full font-black text-xl hover:bg-slate-200 transition-all flex items-center justify-center gap-4 group italic border border-slate-200 uppercase tracking-widest">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[28vw] font-black text-black/[0.03] select-none pointer-events-none uppercase italic leading-none">CLEAN</div>
                </section>
            </div>
                ...
                {selectedImg && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 md:p-20">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="relative w-full h-full">
                            <Image src={selectedImg} alt="Konik Detay" fill className="object-contain" />
                            ...
                            <div className="p-12 md:p-14 text-slate-900 font-sans">
                                {success ? (
                                    <div className="text-center py-10">
                                        ...
                                        <h2 className="text-4xl font-black italic uppercase tracking-tighter leading-tight text-slate-900 text-center">TALEBİNİZ <br /><span className="text-amber-500 font-light italic text-3xl">ALINDI</span></h2>
                                    </div>
                                ) : (
                                    <>
