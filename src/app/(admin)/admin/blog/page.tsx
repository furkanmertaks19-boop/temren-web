...
export default function AdminBlogPage() {
    ...
    const deleteSubscriber = async (id: string) => {
        ...
        } catch (error) { console.error("Abone silme hatası:", error); }
    };
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.url) setCurrentPost((prev: any) => ({ ...prev, image: data.url }));
    ...
    const handleGalleryUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setCurrentPost((prev: any) => ({ ...prev, gallery: [...(prev.gallery || []), ...uploadedUrls] }));
    ...
    return (
        <div className="p-8 bg-[#F8F9FA] min-h-screen text-slate-900">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                <div>
                    <h1 className="text-3xl font-black uppercase italic tracking-tighter text-slate-800">HABER <span className="text-amber-500 font-light text-slate-400">MERKEZİ</span></h1>
                    <div className="flex gap-4 mt-6">
                        <button onClick={() => setActiveTab('posts')} className={`text-[10px] font-black uppercase tracking-widest px-6 py-2.5 rounded-full transition-all ${activeTab === 'posts' ? 'bg-slate-900 text-white shadow-xl scale-105' : 'bg-white text-slate-400 hover:bg-slate-100'}`}>Haberler ({posts.length})</button>
                        <button onClick={() => setActiveTab('newsletter')} className={`text-[10px] font-black uppercase tracking-widest px-6 py-2.5 rounded-full transition-all ${activeTab === 'newsletter' ? 'bg-slate-900 text-white shadow-xl scale-105' : 'bg-white text-slate-400 hover:bg-slate-100'}`}>Aboneler ({subscribers.length})</button>
                    </div>
                </div>
                {activeTab === 'posts' && (
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b">
                            <tr>
                                <th className="p-6 text-[10px] font-black uppercase text-slate-400 tracking-widest">Abonelik Bilgisi</th>
                            ...
                            {subscribers.map((sub: any) => (
                                <tr key={sub._id} className="border-b hover:bg-slate-50/50">
                                    ...
                                    <td className="p-6 text-right">
                                        <button onClick={() => deleteSubscriber(sub._id)} className="p-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all">
                                            <Trash2 size={14} />
                                        </button>
                                    </td>
                                </tr>
                            ...
                            <div className="p-8 overflow-y-auto space-y-8 flex-grow no-scrollbar">
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    <div className="lg:col-span-1 space-y-4">
                                        <label className="text-[10px] font-black uppercase text-slate-400 ml-2 italic">Kapak Fotoğrafı</label>
                                        <div onClick={() => !saving && fileInputRef.current?.click()} className="relative aspect-square bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] flex items-center justify-center cursor-pointer hover:border-amber-500 transition-all overflow-hidden group">
                                            {currentPost.image ? <img src={currentPost.image} className="w-full h-full object-cover" /> : <Upload size={48} className="text-slate-300" />}
                                    ...
                                    <div className="lg:col-span-2 space-y-6">
                                        <div className="grid grid-cols-2 gap-4">
                                            <select className="w-full bg-slate-50 rounded-2xl p-4 font-black text-[10px] uppercase outline-none" value={currentPost.category} onChange={e => setCurrentPost({ ...currentPost, category: e.target.value })}>
                                        ...
                                        <textarea className="w-full bg-slate-50 rounded-2xl p-4 font-medium text-xs outline-none focus:ring-2 ring-amber-500" rows={3} placeholder="Önizleme metni..." value={currentPost.shortDescription} onChange={e => setCurrentPost({ ...currentPost, shortDescription: e.target.value })} />
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        <label className="text-[10px] font-black uppercase text-slate-400 italic">Haber Galerisi</label>
                                        <button onClick={() => !saving && galleryInputRef.current?.click()} className="bg-slate-900 text-white px-5 py-2 rounded-full font-black text-[9px] uppercase hover:bg-amber-500 transition-all shadow-xl">GÖRSEL EKLE</button>
                                    </div>
                                    <div className="grid grid-cols-3 md:grid-cols-6 gap-4 bg-slate-50/50 p-6 rounded-[2.5rem] border-2 border-dashed border-slate-100 min-h-[140px]">
                                        {(currentPost.gallery || []).map((url: string, index: number) => (
                                            <div key={index} className="relative aspect-square rounded-2xl overflow-hidden group shadow-md">
                                                ...
                                                <button onClick={() => setCurrentPost((p: any) => ({ ...p, gallery: p.gallery.filter((_: any, i: number) => i !== index) }))} className="absolute top-2 right-2 p-1.5 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={12} /></button>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="space-y-2 pb-10">
                                    <label className="text-[10px] font-black uppercase text-slate-400 ml-2 italic">Haber Detayı</label>
                                    <div className="quill-editor-container">
                                        <ReactQuill theme="snow" value={currentPost.content} onChange={(val) => setCurrentPost({ ...currentPost, content: val })} modules={quillModules} />
                                    </div>
                                </div>
                            </div>
                            <div className="p-8 bg-slate-50 border-t flex justify-end">
                                <button onClick={handleSave} disabled={saving} className="bg-slate-900 text-white px-14 py-5 rounded-[1.5rem] font-black text-xs uppercase flex items-center gap-2 hover:bg-black shadow-2xl disabled:opacity-50 transition-all">
            ...
            <style jsx global>{`
                .quill-editor-container .ql-toolbar { border-top-left-radius: 1.5rem; border-top-right-radius: 1.5rem; border: 1px solid #e2e8f0; background: #f8fafc; padding: 12px; }
                .quill-editor-container .ql-container { border-bottom-left-radius: 1.5rem; border-bottom-right-radius: 1.5rem; border: 1px solid #e2e8f0; min-height: 400px; font-size: 15px; }
